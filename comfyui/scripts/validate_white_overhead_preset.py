...
def main() -> int:
    ...
    _require(slot.get("status") == "published", "White overhead review slot must be published.")
    ...
    _require(set(modes) == {"preserve_source", "adopt_reference"}, "Both container modes are required.")
    _require(modes["preserve_source"]["container_identity_scope"] == "source_family_and_material_only", "Source-family scope changed.")
