...
> 이 문서는 당시 45도 우드 preset의 과거 유료 smoke 증거입니다. 현재 기본 우드는 `tokyo_a6_relational_scene_hint_v4`이며, A6 및 1:1은 별도 시각 검증 전이므로 이 결과를 현재 기본 preset 검증으로 해석하지 않습니다. 당시의 `880x1100` 전달은 폐기된 계약이며, 현재 4:5는 `1024x1280`을 축소 없이 전달합니다.
